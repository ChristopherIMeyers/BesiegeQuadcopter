...
CurrentEnginePower1 := ThrottleSensitivity * inputThrottle
CurrentEnginePower2 := RotateSensitivity * inputSlider
...
if (DebugEngineMode == true) {
  Tooltip, x %inputX%`, y %inputY%`, s %inputSlider%`, t %inputThrottle%`, e %CurrentEnginePower1%`, e2 %CurrentEnginePower2%`, tx %TargetX%`, ty %TargetY%`, cx %CurrentX%`,cy %CurrentY%`
  return
}
